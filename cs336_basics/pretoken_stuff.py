...
def child_process(args):
    ...
    with open(input_path,"rb") as f:
        f.seek(bounds[0])
        chunk = f.read(bounds[1] - bounds[0]).decode("utf-8", errors="ignore")
    return get_chunk_pretoken_counts(chunk, special_tokens)

def get_chunk_pretoken_counts(chunk:str, special_tokens:list[str]):
    pretokens = []
    pretoken_counts = []
...
def get_pretoken_list(input_path, special_tokens, num_processes):
    # get's the boundaries
    with open(input_path, "rb") as f:
        boundaries = find_chunk_boundaries(f, num_processes, b"<|endoftext|>")
    # given boundaries, spawn children 
    args_list = [((boundaries[i-1], boundaries[i]), input_path, special_tokens) for i in range(1,len(boundaries))]
